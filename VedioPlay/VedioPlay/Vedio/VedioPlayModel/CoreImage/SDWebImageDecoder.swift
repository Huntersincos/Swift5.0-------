    ...
    class func decodedImageWithImage(_ image:UIImage?) -> UIImage?{
      ...
      if ((image?.images) != nil) {
       ...
       let unsupportedColorSpace = (imageColorSpaceModel == .unknown || imageColorSpaceModel == .monochrome  || imageColorSpaceModel == .cmyk || imageColorSpaceModel == .indexed )
        ...
        let context = CGContext.init(data: nil, width: width, height: height, bitsPerComponent: bitsPerComponent, bytesPerRow: bytesPerRow, space: colorspaceRef!, bitmapInfo:CGImageByteOrderInfo.orderDefault.rawValue | CGImageAlphaInfo.noneSkipLast.rawValue )
