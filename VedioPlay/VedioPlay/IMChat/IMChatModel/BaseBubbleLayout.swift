...
func  TimeLabelHeight() -> CGFloat{
    return 20
}

func  NameLabelHeight() -> CGFloat{
    return 20
}
...
func  ContentLabelMaxWidth() -> CGFloat{
    return  CellWidth()-AvatorSize()-2*Margin()-75
}
//let BubbleViewMargin:CGFloat = 10

func  BubbleViewMargin() -> CGFloat{
    return 10
}
//let StateViewSize:CGFloat = 15
func  StateViewSize() -> CGFloat{
    return 15
}

//let StateViewMargin:CGFloat = 5
func  StateViewMargin() -> CGFloat{
    return 5
}
...
class BaseBubbleLayout: NSObject {
   //Cannot use instance member 'AvatorSize' within property initializer; property initializers run before 'self' is available
    ...
    func configWithMessage(_ message:ChatMessageObject?, _ showTime:Bool, _ showName:Bool){
        ...
        let updateTime = (self.message?.timestamp ?? "0") as NSString
        var avatorY:CGFloat = 0
        timeLabelText  = MHPrettyDate.prettyDate(from: Date.init(timeIntervalSince1970: updateTime.doubleValue/1000), with: MHPrettyDateFormat.init(rawValue: MHPrettyDateLongRelativeTime.rawValue))
        ...
        if (message?.messageTranDirection ==  .MessagirectionSend || message?.isCarbonCopy == true){
            avatorViewImage = UIImage.init(named: "img_blueman_nor")
        } else{
            avatorViewImage = UIImage.init(named: "img_greenman_nor")
        }
